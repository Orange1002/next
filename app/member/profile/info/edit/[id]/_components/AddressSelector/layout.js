...
export default function AddressSelector({ value, onChange }) {
  ...
  const [town, setTown] = useState({ name: '', zip: '' })
  ...
  useEffect(() => {
    if (isExternalUpdateRef.current) {
      const newCity = value.city || ''
      const newZip = value.zip || ''
      ...
      // 尋找 town 資料
      const cityData = AddressArray.find((item) => item.city === newCity)
      const matchedTown = cityData?.town.find((t) => t.zip === newZip) || null
      setTown(matchedTown)
    }
  ...
  const towns = city
    ? AddressArray.find((item) => item.city === city)?.town || []
    : []
  ...
  return (
    <div className="d-flex flex-column w-100">
      <div className="d-flex flex-lg-row flex-column justify-content-between">
        ...
        <label className={`${styles.label} w-100 mb-3 me-lg-3 ms-lg-2`}>
          鄉鎮
          <br />
          市區
          <select
      ...
      <div className="d-flex">
        <label className={`${styles.address} w-100 mt-2 mb-2 mx-lg-3`}>
          地址
          <input
            type="text"
            value={address}
