...
export default function SitterDetailPage() {
  ...
  return (
    <main className="bg-light">
      {/* 基本資訊區 */}
      <article className="container-sm py-5" style={{ maxWidth: '1440px' }}>
        <div className="row g-5 align-items-center justify-content-center ">
          <figure className="col-12 col-lg-6 p-5">
            <Image
              src={sitter?.avatar_url || '/no-image.png'}
              alt="Pet Sitter Profile"
              className="img-fluid rounded justify-content-center"
              width={100}
              height={100}
              style={{ width: 'auto' }}
            />
          ...
          <section className="col-12 col-lg-6 profile-height border-bottom border-2">
            ...
            <section className="d-flex flex-column gap-2 m-3">
              <div className="d-flex align-items-center gap-2">
                <span style={{ fontSize: '1.75rem', color: '#f5b301' }}>
                  ⭐
                </span>
      ...
      <section className="container pb-5">
        ...
        >
          {sitter?.reviews?.map((review, i) => (
            <SwiperSlide key={i}>
              <div className="border rounded p-3 h-100 bg-light shadow-sm">
                <StarRating
                  value={review.rating}
                  readOnly
                  fillColor="#f5b301"
                ...
                <p className="text-secondary small mt-2">{review.comment}</p>
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
      ...
      {/* 留下評論區塊 */}
      {canReview && (
        <section className="container my-5 p-4 border rounded bg-white">
